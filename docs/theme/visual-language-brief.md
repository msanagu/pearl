...
visual-generation tool ("Claude Design") to produce a visual language _as token
values_ that translate directly into the design system's config. It doubles as
a record of how the token schema was pressure-tested.

The schema slots below are being QA'd one group at a time (the decisions behind
them are summarized in [DECISIONS.md](../../DECISIONS.md)). Sections marked
...
I have a small React design system built as a _visual-agnostic engine_: the same
...
Instantiate the _exact same semantic slots_ three ways so identical components
visibly transform:
...
- Sentiment colors: keyed by valence (`positive` / `negative` / `warn` / `info`),
  ...
  See the token-conventions entry in [DECISIONS.md](../../DECISIONS.md).
