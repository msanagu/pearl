...
- [manifest/base.json](./manifest/base.json): ${baseManifest.entities.length} theme-agnostic Component entities (props/types only — real usage examples live in each component's own file below, not embedded here). Also carries two cross-cutting fields, both worth reading before generating anything: \`overrideContract\` — the real mechanism for extending past a component's documented variants (data-attribute targeting, never inline styles), and the expectation that doing so gets flagged, not shipped silently. \`tokenSemantics\` — what each sentiment-color sub-field (\`surface\`/\`border\`/\`text\`/\`icon\`) is actually for; \`icon\` in particular is deliberately desaturated for glyph use, not a general-purpose strong fill color.
${themeLines}
