...
const meta: Meta<typeof Text> = {
  ...
  argTypes: {
    typeScale: {
      ...
      description:
        'Size band: fontSize + lineHeight + tracking, and the default face when no `role` is set.',
    },
    role: {
      control: 'select',
    ...
    measure: {
      ...
      description:
        'Opt-in prose line-length cap. Unset means fill the container.',
    },
  },
};
export default meta;
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <Text typeScale="headingSm" as="h2">
        headingSm as h2 (prominent section)
      </Text>
      <Text typeScale="bodyMd" as="h2">
        bodyMd as h2 (subtle section label)
      </Text>
    </div>
  ),
};
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <Text prominence="subtle">
        Subtle prominence — captions, metadata, helper text.
      </Text>
    </div>
  ),
};
 ...
 */
/**
 * One specimen string shared by every block below — the story only reads if the
 * prose is identical across steps.
 */
