 ...
 *
 * Landing vs. docs nav (Porsche v4 reference): the top bar is deliberately
...
export function Hero({
  ...
  return (
    // GAP — full-bleed/constrained-content split has no primitive today. Each
    // bordered band is its own edge-to-edge wrapper with an inner maxWidth
    // container, so borders reach the viewport edge instead of stopping at the
    // content measure.
    <Stack style={{ minHeight: showNav ? '100vh' : undefined }}>
      {showNav && (
        // GAP — no Nav/Header layout primitive. Utility chrome only.
        <Row style={{ borderBottom: `1px solid ${color.border}`, padding: `${space.md} ${space.xl}` }}>
      ...
      <Row
        ...
        align="center"
        wrap
        style={{ flex: 1, maxWidth: 1200, margin: '0 auto', padding: `${space['2xl']} ${space.xl}`, width: '100%' }}
      >
        {/*
      ...
      <Row style={{ borderTop: `1px solid ${color.border}`, borderBottom: `1px solid ${color.border}` }}>
